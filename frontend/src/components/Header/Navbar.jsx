  ...
  return (
    <>
      ...
      <header
        ...
        dir={dir}
        className={`studio-navbar fixed top-0 left-0 right-0 z-50 transition-all duration-300 transform ${
      ...
      >
        {/* Left / Start Section */}
        <div className="navbar-identity flex items-center gap-2.5">
          <button
          ...
          <div className="relative">
            ...
            {botDropdownOpen && (
              <div className="navbar-bot-menu absolute top-full mt-1 w-56 bg-surface border border-border rounded-xl shadow-2xl p-1 z-50 backdrop-blur-lg">
          ...
          {currentBot && (
            <button
              ...
              title={t('navbar.bot_settings')} aria-label={t('navbar.bot_settings')}
            >
              <SettingsIcon size={14} className="text-muted hover:text-foreground" />
              <span className="hidden sm:inline">{t('navbar.bot_settings')}</span>
            </button>
          )}
          ...
          {currentBot && (
            <button
              ...
              title={isRunning ? (t('navbar.stop_bot')) : (t('navbar.run_bot'))}
            >
              {isRunning ? (
                <>
                  <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                  <Pause size={13} className="fill-current" />
                  <span>{t('navbar.running')}</span>
                </>
              ) : (
                <>
                  <Play size={13} className="fill-current" />
                  <span>{t('navbar.run_bot')}</span>
                </>
              )}
            </button>
          )}
          ...
          {isDirty && (
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-warning/10 border border-warning/30 text-warning text-[11px] font-medium">
              <AlertCircle size={12} />
          ...
          {updateInfo?.has_update && (
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-success/15 border border-success/30 text-success text-[11px] font-medium">
              <Sparkles size={12} />
              <span>{t('navbar.update_available')} ({t('common.version_label', { version: updateInfo.latest_version })})</span>
            </div>
          )}
        </div>
        ...
        <div className="navbar-actions flex items-center gap-2">
          ...
          <button
            ...
            title={t('navbar.plugins')} aria-label={t('navbar.plugins')}
          >
            <Puzzle size={14} />
        ...
        <div className="navbar-quick-controls flex items-center gap-2">
          ...
          <button
            ...
            title={t('navbar.theme_toggle')} aria-label={t('navbar.theme_toggle')}
          >
          ...
          <button
            ...
            className="p-1.5 rounded-lg bg-surface-secondary hover:bg-surface-tertiary border border-border text-foreground"
            title={t('common.fullscreen')} aria-label={t('common.fullscreen')}
          >
          ...
          <button
            ...
            title={t(isPinned ? 'navbar.unpin' : 'navbar.pin')} aria-label={t(isPinned ? 'navbar.unpin' : 'navbar.pin')}
          >
